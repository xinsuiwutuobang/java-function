...
import com.yangfei.functioneasyexcel.anno.CustomMerge;
import lombok.Data;
...
public class OrderData {
    /*
     * CustomMerge 自定义注解，用于判断是否需要合并以及合并的主键
     * isPk = true 单元格合并时，用于作为主键来对比是否需要合并
     * needMerge 判断是否需要合并
     * */
    @ExcelProperty(value = "订单ID")
    @ContentStyle(horizontalAlignment = HorizontalAlignmentEnum.CENTER_SELECTION)
    @ColumnWidth(10)
    @CustomMerge(needMerge = true, isPk = true)
    private String id;
    @ExcelProperty(value = "订单编码")
    @ColumnWidth(20)
    @CustomMerge(needMerge = true, isPk = false)
