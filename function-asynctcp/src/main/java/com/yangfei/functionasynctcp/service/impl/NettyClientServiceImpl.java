package com.yangfei.functionasynctcp.service.impl;

import com.yangfei.functionasynctcp.client.NettyClient;
import com.yangfei.functionasynctcp.client.SyncFuture;
...
public class NettyClientServiceImpl implements NettyClientService {
    ...
    public String sendSyncMsg(String text, String dataId, String serviceId) {
        ...
        // 放入缓存中
        futureCache.put(uuid, syncFuture);
        ...
        // 发送同步消息
        String result = nettyClient.sendSyncMsg(uuid,text, syncFuture);

        return result;
    }

    @Override
    public void ackSyncMsg(String uuid,String msg) {

        LOGGER.info("ACK确认信息uuid:{},msg{}",uuid,msg);

       /* JSONObject object =JSON.parseObject(msg);
        String dataId = object.getString("dataId");*/

        // 从缓存中获取数据
        SyncFuture<String> syncFuture = futureCache.getIfPresent(uuid);
        // 如果不为null, 则通知返回
        if(syncFuture != null) {
            syncFuture.setResponse(msg);
            //主动释放
            futureCache.invalidate(uuid);
        }
    }
